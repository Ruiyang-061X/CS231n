...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss = np.sum(- np.log(p)) / X.shape[0] + reg * np.sum(np.square(W1)) + reg * np.sum(np.square(W2))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    dscores = np.exp(shift_scores) / np.array([np.sum(np.exp(shift_scores), axis=1)]).T
    dscores[range(X.shape[0]), y] -= 1
    dscores /= X.shape[0]
    grads['W2'] = np.dot(h_output.T, dscores) + reg * 2 * W2
    grads['b2'] = np.sum(dscores, axis=0)
    dh_output = np.dot(dscores, W2.T)
    grads['W1'] = np.dot(X.T, (h_output > 0) * dh_output) + reg * 2 * W1
    grads['b1'] = np.sum((h_output > 0) * dh_output, axis=0)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
